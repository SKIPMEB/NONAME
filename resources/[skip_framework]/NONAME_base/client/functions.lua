...
function NONAME.Start()
    Citizen.CreateThread(function()
        while true do
            if NetworkIsSessionStarted() then
                TriggerEvent('NONAME_base:Start')
                TriggerServerEvent('NONAME_base:ServerStart')
                break
            end
        end
    end)
...
--player
function NONAME.Functions.GetPlayerData(cb)
    if not cb then return NONAME.PlayerData end
    cb(NONAME.PlayerData)
end
...
function NONAME.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
    local flags = upperbodyOnly and 16 or 0
    local runTime = duration or -1
    NONAME.Functions.RequestAnimDict(animDict)
    TaskPlayAnim(PlayerPedId(), animDict, animName, 8.0, 1.0, runTime, flags, 0.0, false, false, true)
    RemoveAnimDict(animDict)
end

function NONAME.Functions.LoadModel(model)
...
function NONAME.Functions.LoadAnimSet(animSet)
...
-- Getters
function NONAME.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end

function NONAME.Functions.GetObjects()
    return GetGamePool('CObject')
end


function NONAME.Functions.GetPlayers()
    return GetActivePlayers()
end



function NONAME.Functions.GetClosestPed(coords, ignoreList)
    ...
    local peds = NONAME.Functions.GetPeds(ignoreList)
...
function NONAME.Functions.IsWearingGloves()
    local ped = PlayerPedId()
    local armIndex = GetPedDrawableVariation(ped, 3)
    local model = GetEntityModel(ped)
    if model == "mp_m_freemode_01" then
        if NONAME.Shared.MaleNoGloves[armIndex] then
            return false
        end
    else
        if NONAME.Shared.FemaleNoGloves[armIndex] then
            return false
        end
    end
    return true
...
function NONAME.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = NONAME.Functions.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function NONAME.Functions.GetPlayersFromCoords(coords, distance)
...
function NONAME.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function NONAME.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function NONAME.Functions.GetClosestBone(entity, list)
...
function NONAME.Functions.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function NONAME.Functions.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
    local modelHash = type(model) == 'string' and GetHashKey(model) or model
    local bone = GetPedBoneIndex(ped, boneId)
    NONAME.Functions.LoadModel(modelHash)
...
function NONAME.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    local ped = PlayerPedId()
    ...
    NONAME.Functions.LoadModel(model)
...
function NONAME.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function NONAME.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return NONAME.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end

function NONAME.Functions.GetVehicleLabel(vehicle)
    if vehicle == nil or vehicle == 0 then return end
    return GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
end

function NONAME.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function NONAME.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            plate = NONAME.Functions.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),
            bodyHealth = NONAME.Shared.Round(GetVehicleBodyHealth(vehicle), 0.1),
            engineHealth = NONAME.Shared.Round(GetVehicleEngineHealth(vehicle), 0.1),
            tankHealth = NONAME.Shared.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = NONAME.Shared.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = NONAME.Shared.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = NONAME.Shared.Round(GetVehicleOilLevel(vehicle), 0.1),
            color1 = colorPrimary,
...
function NONAME.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function NONAME.Functions.LoadParticleDictionary(dictionary)
...
function NONAME.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(PlayerPedId())
    end
    NONAME.Functions.LoadParticleDictionary(dict)
...
function NONAME.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    NONAME.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
